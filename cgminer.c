...
static struct opt_table opt_config_table[] = {
...
#ifdef USE_HASHFAST
	OPT_WITHOUT_ARG("--hfa-init-only",
			opt_set_bool, &opt_hfa_init_only,
			opt_hidden),
	...
	OPT_WITHOUT_ARG("--hfa-hash-audit",
			opt_set_bool, &opt_hfa_hash_audit,
			"Log every nonce found with magnitude of hash collision."),
	OPT_WITH_ARG("--hfa-set-search-difficulty",
		     set_int_0_to_255, opt_show_intval, &opt_hfa_audit_sd,
		     "Lower search difficulty to force more nonces to be returned."),
#endif
...
static bool submit_upstream_work(struct work *work, CURL *curl, bool resubmit)
{
	...
	// Fix: Very unconservative.  Maybe this function does something useful beyond
	//      informing the pool of results!
	if( strcmp(pool->rpc_url, "Benchmark") == 0 || strcmp(pool->rpc_url, "Benchfile") == 0 )
	  return true;
