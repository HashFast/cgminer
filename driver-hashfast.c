...
static void report_hash_audit_datapoint(struct cgpu_info *hashfast, unsigned char hash[32],
					double device_diff)
{
	int i;
	int zeros;
	...
	rslt = gettimeofday(&now, (struct timezone *) NULL);
	if(rslt == -1) {
		applog(LOG_WARNING, "gettimeofday() failed. errno: %d", errno);
		now.tv_sec = 0;
		now.tv_usec = 0;
	}

	/* Reproduce the number of leading zeroes to look for based on diff
	 * as hfa_scanwork() did it.
	 * Diff 1 = 32, Diff 2 = 33, Diff 4 = 34 etc. */
	intdiff = (uint64_t) device_diff;
	for (i = 31; intdiff; i++, intdiff >>= 1);
	search_difficulty = i;
	if(opt_hfa_hash_audit && opt_hfa_audit_sd >= 0 && i > opt_hfa_audit_sd)
		search_difficulty = opt_hfa_audit_sd;
	...
	applog(LOG_WARNING, "Hash rate audit: %s %d: %ld.%06ld Search Difficulty: %u Zeros: %d",
		hashfast->drv->name, hashfast->device_id,
		(long) now.tv_sec, (long) now.tv_usec, search_difficulty, zeros);
}
...
restart:
	...
	while (jobs-- > 0) {
		...
		if(opt_hfa_hash_audit && opt_hfa_audit_sd >= 0 && i > opt_hfa_audit_sd)
  		   op_hash_data.search_difficulty = opt_hfa_audit_sd;
...
static void hfa_statline_before(char *buf, size_t bufsiz, struct cgpu_info *hashfast)
{
	...
	tailsprintf(buf, bufsiz, "%3dM", info->base_clock);
	tailsprintf(buf, bufsiz, " %1d%%F", info->fanspeed);

	for (i = 0; i < info->asic_count; i++) {
		int j;

		d = &info->die_status[i];
		// G1 die has six voltage sample points, see "struct hf_g1_monitor" in hf_protocol.h.
		for (j = 0; j < 6; j++) {
